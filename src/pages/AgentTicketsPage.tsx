...
const AgentTicketQueue = () => {
  ...
  const handleAssignTickets = async (
    ...
    for (const ticketId of selectedTicketIds) {
      await dispatch(
        updateTicket({
          ticketId,
          updates: { assigned_to: agentIdFromModal }, // Fixed: Use agentIdFromModal instead of agentName
        }),
      );
    }
  ...
  const handleAssignToMe = async (ticketId: string) => {
    ...
    try {
      ...
      dispatch(fetchTickets({ status: "", priority: "" })); // Refresh tickets
    } catch (error) {
      toast.error("Failed to assign ticket to yourself.");
      console.error("Failed to assign ticket to self:", error);
    }
  };
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-medium border ${styles[status as keyof typeof styles] || "bg-gray-100 text-gray-700"}`}
      >
        {status === "in-progress"
          ? "In Progress"
  ...
  const PriorityBadge = ({ priority }: { priority: string }) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-medium border ${styles[priority as keyof typeof styles]}`}
      >
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    const matchesAssigned =
      ...
      (filters.assignedTo.includes("me") && ticket.assigned_to === user?.id) || // Changed to user?.id
    ...
    const matchesView =
      ...
      (viewMode === "assigned-to-me" && ticket.assigned_to === user?.id) || // Changed to user?.id
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setFilters({
                            ...filters,
                            assignedTo: [...filters.assignedTo, "unassigned"],
                          });
                        } else {
                          setFilters({
                            ...filters,
                            assignedTo: filters.assignedTo.filter(
                              (a) => a !== "unassigned",
                            ),
                          });
                        }
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setFilters({
                            ...filters,
                            assignedTo: [...filters.assignedTo, "me"],
                          });
                        } else {
                          setFilters({
                            ...filters,
        ...
        {!isLoading && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                          ...
                          <div className="flex items-center">
                            <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center mr-2">
                              <span className="text-xs font-medium text-gray-700">
                                {(customerMap[ticket.customer_id]?.name || "C")
                                  .split(" ")
                                  .map((n: string) => n[0])
                                  .join("")}
                              </span>
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {customerMap[ticket.customer_id]?.email || ""}
                              </p>
                            </div>
                          </div>
                        </td>
                          ...
                          {ticket.assigned_to === user?.id ? (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              ticket.assigned_to === "Unassigned" ? (
                                <div className="flex items-center gap-2">
                                  <button
                                    className="px-2 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-medium hover:bg-gray-200 transition-colors flex items-center whitespace-nowrap"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                  ...
                                  <button
                                    className="px-2 py-1 bg-purple-100 text-purple-600 rounded-lg text-xs font-medium hover:bg-purple-200 transition-colors flex items-center whitespace-nowrap"
                                    onClick={(e) => {
                                      e.stopPropagation();
            ...
            <AssignAgentsModal
              ...
              ticketIds={selectedTicketIds} // Changed to selectedTicketIds
