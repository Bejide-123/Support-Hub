...
import {
  Search, ChevronRight, Clock, Download, UserPlus, Inbox,
  CheckSquare, X, SlidersHorizontal, RefreshCw, Circle,
  CircleDot, CheckCircle2, MinusCircle,
  AlertTriangle,
} from "lucide-react";
...
const STATUS_CFG: Record<string, { label: string; cls: string; dot: string; icon: React.ReactNode }> = {
  open:          { label: "Open",        dot: "bg-sky-500",     cls: "text-sky-600 bg-sky-50 border-sky-200",           icon: <Circle size={10} className="fill-sky-500 text-sky-500" /> },
  "in-progress": { label: "In Progress", dot: "bg-amber-400",   cls: "text-amber-600 bg-amber-50 border-amber-200",     icon: <CircleDot size={10} className="text-amber-500" /> },
  resolved:      { label: "Resolved",    dot: "bg-emerald-500", cls: "text-emerald-600 bg-emerald-50 border-emerald-200", icon: <CheckCircle2 size={10} className="text-emerald-500" /> },
  closed:        { label: "Closed",      dot: "bg-gray-400",    cls: "text-gray-400 bg-gray-100 border-gray-200",       icon: <MinusCircle size={10} className="text-gray-400" /> },
  urgent:        { label: "Urgent",      dot: "bg-red-500",     cls: "text-red-600 bg-red-50 border-red-200",           icon: <AlertTriangle size={10} className="text-red-500" /> },
};

const PRIORITY_CFG: Record<string, { label: string; cls: string; bar: string; text: string }> = {
  urgent: { label: "Urgent", cls: "text-red-600 bg-red-50 border-red-200",       bar: "bg-red-500",    text: "text-red-600"    },
  high:   { label: "High",   cls: "text-orange-600 bg-orange-50 border-orange-200", bar: "bg-orange-400", text: "text-orange-500" },
  medium: { label: "Medium", cls: "text-amber-600 bg-amber-50 border-amber-200", bar: "bg-amber-400",  text: "text-amber-500"  },
  low:    { label: "Low",    cls: "text-emerald-600 bg-emerald-50 border-emerald-200", bar: "bg-emerald-400", text: "text-emerald-600" },
};
...
const AssignedAgentName = ({ userId }: { userId: string }) => {
  ...
  const agent    = useAppSelector(s => selectUserById(s, userId));

  useEffect(() => {
    if (userId && userId !== "Unassigned" && !agent) {
      dispatch(getUserById(userId));
    }
  }, [userId, agent, dispatch]);
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-violet-400 to-indigo-500 flex items-center justify-center text-white text-[9px] font-extrabold flex-shrink-0">
        {initials}
      </div>
      <span className="text-sm text-gray-700 font-semibold truncate max-w-[100px]">
        {agent.name || "Unknown"}
      </span>
    </div>
  );
...
const FilterCheck = ({
  ...
  <label className="flex items-center gap-2.5 cursor-pointer group">
    ...
    <span className="text-sm text-gray-600 capitalize">{label === "in-progress" ? "In Progress" : label}</span>
  </label>
);
...
const AgentTicketQueue = () => {
  ...
  const handleAssignTickets = async (agentId: string) => {
    for (const tid of selectedTicketIds) {
      await dispatch(updateTicket({ ticketId: tid, updates: { assigned_to: agentId } }));
    }
    dispatch(fetchTickets({ status: "", priority: "" }));
  ...
  // Fixed toggleAll - using 'filtered' instead of 'filteredTickets'
  const toggleAll = () =>
    setSelectedTickets(selectedTickets.length === filtered.length ? [] : filtered.map(t => t.id));

  const clearFilters = () => {
    setFilters({ status: [], priority: [], category: [], assignedTo: [] });
    setViewMode("all"); setSearchQuery(""); setSelectedTickets([]);
  };

  const toggleFilter = (key: "status" | "priority" | "category" | "assignedTo", val: string) =>
    setFilters(prev => ({
      ...prev,
  ...
  const filtered = tickets
    .filter(t => {
      ...
      return (
        (!q || t.subject.toLowerCase().includes(q) || t.id.toLowerCase().includes(q) || cName.toLowerCase().includes(q)) &&
        (!filters.status.length    || filters.status.includes(t.status)) &&
        (!filters.priority.length  || filters.priority.includes(t.priority)) &&
        (!filters.category.length  || filters.category.includes(t.category || "")) &&
        (viewMode === "all" ||
         (viewMode === "unassigned"    && (!t.assigned_to || t.assigned_to === "Unassigned")) ||
  ...
  const activeFilterCount = filters.status.length + filters.priority.length + filters.category.length + filters.assignedTo.length;
    ...
    <div className="min-h-screen bg-[#f4f5f7]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-6 mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-xl font-extrabold text-gray-900">Ticket Queue</h1>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={() => { clearFilters(); dispatch(fetchTickets({ status: "", priority: "" })); }}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          {[
            { label: "Total Tickets", value: stats.total,      accent: "bg-violet-500", delta: null,   up: true  },
            { label: "Urgent",        value: stats.urgent,     accent: "bg-red-500",    delta: null,   up: false },
            { label: "Unassigned",    value: stats.unassigned, accent: "bg-amber-500",  delta: null,   up: false },
            { label: "Open",          value: stats.open,       accent: "bg-sky-500",    delta: "+12%", up: true  },
          ].map((s, i) => (
        ...
        {showFilters && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
            <div className="flex items-center justify-between mb-5">
              <span className="font-bold text-gray-900 text-sm">Filter Tickets</span>
              <button onClick={clearFilters} className="text-xs font-bold text-violet-600 hover:text-violet-700 transition-colors">
                Clear all
              </button>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {["open","in-progress","resolved","closed"].map(s => (
                    <FilterCheck key={s} label={s} checked={filters.status.includes(s)} onChange={() => toggleFilter("status", s)} />
                  ))}
                </div>
              </div>
              {/* Priority */}
              <div>
                ...
                <div className="space-y-2.5">
                  {["urgent","high","medium","low"].map(p => (
                    <FilterCheck key={p} label={p} checked={filters.priority.includes(p)} onChange={() => toggleFilter("priority", p)} />
                  ))}
                </div>
              </div>
              {/* Category */}
              <div>
                ...
                <div className="space-y-2.5">
                  {["Billing","Technical Issue","Feature Request","Account Management","API"].map(c => (
                    <FilterCheck key={c} label={c} checked={filters.category.includes(c)} onChange={() => toggleFilter("category", c)} />
                  ))}
                </div>
              </div>
              {/* Assignment */}
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Assignment</p>
                <div className="space-y-2.5">
                  {[{ key: "unassigned", label: "Unassigned" }, { key: "me", label: "Assigned to me" }].map(a => (
                    <FilterCheck key={a.key} label={a.label} checked={filters.assignedTo.includes(a.key)} onChange={() => toggleFilter("assignedTo", a.key)} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            {selectedTickets.length > 0 && (
              <div className="flex items-center gap-3 bg-violet-50 border border-violet-200 px-4 py-2.5 rounded-xl">
                ...
                <span className="text-sm font-bold text-violet-700">{selectedTickets.length} selected</span>
                <button
                  onClick={() => { setSelectedTicketIds(selectedTickets); setShowAssignModal(true); }}
                  className="px-3 py-1.5 bg-violet-600 text-white rounded-lg text-xs font-bold hover:bg-violet-700 transition-colors">
                  Assign
                </button>
                <button onClick={() => setSelectedTickets([])} className="p-1.5 hover:bg-violet-200 rounded-lg transition-colors">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/60">
                    <th className="px-5 py-3.5 text-left w-10">
                      <div
                        ...
                        className={`w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer transition-all ${
                          selectedTickets.length === filtered.length && filtered.length > 0
                    ...
                    {["Ticket ID","Subject","Customer","Status","Priority","Assigned","Updated",""].map((h, i) => (
                      <th key={i} className="px-4 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer group transition-colors ${
                          isSelected ? "bg-violet-50/60" : isUrgent ? "bg-red-50/30 hover:bg-red-50/60" : "hover:bg-gray-50/80"
                        }`}>

                        {/* Checkbox */}
                        <td className="px-5 py-4" onClick={e => { e.stopPropagation(); toggleTicket(ticket.id); }}>
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-gray-800 truncate">{customer?.name || "Unknown"}</p>
                          ...
                          {ticket.assigned_to === user?.id ? (
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-[9px] font-extrabold flex-shrink-0">
                                {user?.name ? user.name.split(" ").map((n: string) => n[0]).join("").slice(0,2).toUpperCase() : "ME"}
                              </div>
                              <span className="text-sm text-violet-700 font-bold truncate max-w-[80px]">
                                {user?.name?.split(" ")[0] || "You"}
                              </span>
                              <span className="text-[10px] font-bold bg-violet-100 text-violet-600 px-1.5 py-0.5 rounded-full flex-shrink-0">You</span>
                            </div>
                          ) : !ticket.assigned_to || ticket.assigned_to === "Unassigned" ? (
                            <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                              <button
                                onClick={e => { setSelectedTicketIds([ticket.id]); setShowAssignModal(true); e.stopPropagation(); }}
            ...
            {filtered.length > 0 && (
              <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <p className="text-sm text-gray-500">
                  Showing <span className="font-bold text-gray-700">1</span>–<span className="font-bold text-gray-700">{filtered.length}</span> of <span className="font-bold text-gray-700">{filtered.length}</span>
                </p>
