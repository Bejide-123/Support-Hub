            ...
            >
              ...
              <div className="px-2.5 py-2 flex items-center justify-between gap-1">
                ...
                <a
                  ...
                  className="p-1.5 text-gray-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-colors flex-shrink-0"
                >
                  <Download size={12} />
                </a>
              </div>
...
const CANNED = [
  {
    ...
    content:
      'To reset your password, visit the login page and click "Forgot Password". You\'ll receive an email within 5 minutes.',
  },
  {
    title: "Refund Processed",
    content:
      "Your refund has been processed. The amount will reflect in your account within 3–5 business days.",
  },
  {
    title: "Technical Issue",
    content:
      "I apologize for the inconvenience. Our engineering team has been notified and is working on a fix. I will update you as soon as it's resolved.",
  },
  {
    title: "Billing Question",
    content:
      "I understand your concern about billing. Let me walk you through how our billing cycle works...",
  },
];
...
const PriorityDot = ({ priority }: { priority: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-[11px] font-semibold ${cfg.text}`}
    >
...
const Avatar = ({
  ...
  <div
    className={`w-${size} h-${size} rounded-full ${gradient} flex items-center justify-center text-white font-bold text-xs flex-shrink-0`}
  >
...
const Dropdown = ({
  ...
  open ? (
    <div className="absolute top-full right-0 mt-1.5 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-50 min-w-[160px]">
      {children}
    </div>
  ) : null;
...
const AgentTicketDetailPage = () => {
  ...
  const [tab, setTab] = useState<"conversation" | "internal" | "activity">(
    "conversation",
  );
  ...
  const toggleDd = (name: string) =>
    setDd((prev) => (prev === name ? null : name));
  ...
  const assignedAgent = useAppSelector((s) =>
    ticket?.assigned_to ? selectUserById(s, ticket.assigned_to) : null,
  );
  const customer = useAppSelector((s) =>
    ticket?.customer_id ? selectUserById(s, ticket.customer_id) : null,
  );
  ...
  const userTickets = useAppSelector((state) =>
    ticket?.customer_id ? selectUserTickets(state, ticket.customer_id) : [],
  );
  const customerTicketCount = userTickets.length;
  ...
  const markResolved = async () => {
    try {
      await dispatch(
        updateTicket({ ticketId: id!, updates: { status: "resolved" } }),
      ).unwrap();
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const loadingId = showLoading("Sending…");
    setSending(true);
    try {
      await dispatch(
        addMessage({
          ticketId: id!,
          message: replyTo ? `@${replyTo.author} ${msg}` : msg,
  ...
  const doSnooze = async (dur: string) => {
    ...
    try {
      await dispatch(
        updateTicket({
          ticketId: id!,
          updates: { snoozed_until: d.toISOString() },
        }),
      ).unwrap();
      showSuccess(`Snoozed until ${d.toLocaleTimeString()}`);
    } catch {
      showError("Failed");
    }
    setSnooze(false);
          ...
          <div className="bg-red-600 text-white">
            <div className="max-w-7xl mx-auto px-6 py-2.5 flex items-center justify-between">
              <div className="flex items-center gap-3 text-sm">
                ...
                <button className="text-xs underline text-red-200 hover:text-white">
                  Escalate now
                </button>
              </div>
              <button
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-2 text-gray-400 hover:text-violet-600 hover:bg-white rounded-lg transition-all border border-transparent hover:border-gray-200"
            >
              <RefreshCw size={15} />
            </button>
            <button
          ...
          <div className="space-y-4">
              ...
              {editing ? (
                <div className="space-y-3">
                  <input
                    ...
                    placeholder="Subject"
                    className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-violet-400 outline-none"
                  />
                  <select
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 bg-violet-600 text-white text-xs font-bold rounded-lg hover:bg-violet-700 flex items-center gap-1.5"
                    >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[11px] text-gray-400 flex items-center gap-1">
                          <Clock size={10} /> {formatDate(ticket.updated_at)}
                        </span>
                      </div>
                    </div>
                          ...
                          <div className="absolute right-0 mt-1.5 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-50 min-w-[160px]">
                            {[
                              ...
                              <button
                                key={k}
                                onClick={() => doSnooze(k)}
                                className="w-full flex items-center gap-2 px-3.5 py-2 text-xs text-gray-700 hover:bg-gray-50"
                              >
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="flex border-b border-gray-100 bg-gray-50/60">
                {(["conversation", "internal", "activity"] as const).map(
                  (t) => (
                    <button
                      key={t}
                      onClick={() => setTab(t)}
                      className={`relative px-5 py-3.5 text-xs font-bold transition-colors capitalize flex items-center gap-1.5 ${tab === t ? "text-violet-600" : "text-gray-400 hover:text-gray-600"}`}
                    >
                      ...
                      {t === "conversation"
                        ...
                        : t === "internal"
                          ...
                          : "Activity"}
                      {t === "conversation" && (
                        <span className="px-1.5 py-0.5 bg-gray-200 text-gray-500 rounded-full text-[10px]">
                          {convMsgs.length}
                        </span>
                      )}
                      {t === "internal" && intlMsgs.length > 0 && (
                        <span className="px-1.5 py-0.5 bg-amber-100 text-amber-600 rounded-full text-[10px]">
                          {intlMsgs.length}
                        </span>
                      )}
                      {tab === t && (
              ...
              {tab === "conversation" && (
                <>
                  <div
                    className="h-[440px] overflow-y-auto px-5 py-5 space-y-5"
                    style={{
                      scrollbarWidth: "thin",
                  ...
                  >
                    {convMsgs.length === 0 ? (
                      ...
                      convMsgs.map((m, i) => {
                        ...
                        const firstInGroup =
                          i === 0 || convMsgs[i - 1]?.author_id !== m.author_id;
                        return (
                          <div
                            key={m.id}
                          ...
                          >
                            ...
                            <div
                              className={`flex flex-col max-w-[70%] ${isAgent ? "items-end" : "items-start"}`}
                            >
                              {firstInGroup && (
                                <div
                                  className={`flex items-center gap-2 mb-1.5 ${isAgent ? "flex-row-reverse" : ""}`}
                                >
                              ...
                              <div
                                className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${isAgent ? "bg-gradient-to-br from-violet-600 to-indigo-600 text-white rounded-tr-sm" : "bg-gray-100 text-gray-800 rounded-tl-sm"}`}
                              >
                  ...
                  {files.length > 0 && (
                    <div className="px-5 py-2.5 border-t border-gray-100 flex flex-wrap gap-2">
                      {files.map((f, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-1.5 bg-gray-100 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-gray-600"
                        >
                          <FileText size={11} />
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden focus-within:border-violet-400 focus-within:ring-2 focus-within:ring-violet-100 transition-all">
                        ...
                        <div className="flex items-center justify-between px-3 py-2 border-t border-gray-100">
                          ...
                          <button
                            type="submit"
                            disabled={
                              (!msg.trim() && files.length === 0) || sending
                            }
              ...
              {tab === "internal" && (
                <div className="p-5">
                  <div
                    className="space-y-3 max-h-[360px] overflow-y-auto mb-4"
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl overflow-hidden">
                        <div className="px-3 py-2 bg-amber-100/50 border-b border-amber-200">
                          <span className="text-[11px] font-bold text-amber-700 flex items-center gap-1.5">
                            <Lock size={11} /> Internal — only visible to agents
                          </span>
                        </div>
                        <textarea
                          value={note}
                        ...
                        <div className="flex items-center justify-end gap-2 px-3 py-2 border-t border-amber-200">
                          ...
                          <button
                            type="submit"
                            disabled={!note.trim()}
                            className="flex items-center gap-1.5 px-4 py-1.5 bg-amber-600 text-white text-xs font-bold rounded-lg hover:bg-amber-700 disabled:opacity-40"
                          >
              ...
              {tab === "activity" && (
                <div
                  className="p-5 space-y-2 max-h-[480px] overflow-y-auto"
                  style={{ scrollbarWidth: "thin" }}
                >
                  {[
                    {
                      action: "Ticket created",
                      user: customer?.name || ticket.customer_id,
                      time: ticket.created_at,
                      color: "bg-sky-500",
                    },
                    {
                      action: `Status set to ${ticket.status}`,
                      user: "System",
                  ...
                  >
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">
                        {assignedAgent ? "Support Agent" : "Click to assign"}
                      </p>
                    </div>
                    <ChevronDown
                      size={13}
                    ...
                    <Dropdown open={dd === "status"}>
                      {["Open", "In Progress", "Resolved", "Closed"].map(
                        (s) => (
                          ...
                          >
                            {
                              STATUS_CFG[s.toLowerCase().replace(" ", "-")]
                                ?.icon
                            }{" "}
                            {s}
                          </button>
                  ...
                  <div className="relative">
                    ...
                    <Dropdown open={dd === "priority"}>
                      {["urgent", "high", "medium", "low"].map((p) => (
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${PRIORITY_CFG[p].bar}`}
                          />
                          {p}
                        </button>
                      ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(ticket.tags || []).map((tag: string, i: number) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 text-gray-700 rounded-lg text-[11px] font-medium"
                    >
                      {tag}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="space-y-2.5 border-t border-gray-100 pt-3.5">
                {[
                  ...
                  {
                    icon: <Phone size={12} />,
                    val: customer?.phone || "No phone provided",
                  },
                  { icon: <Ticket size={12} />, val: `${customerTicketCount} total ${customerTicketCount === 1 ? "ticket" : "tickets"}` },
                ].map((row, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    ...
                    {row.href ? (
                      <a
                        href={row.href}
                        className="text-xs text-gray-600 hover:text-violet-600 truncate transition-colors"
                      >
                        {row.val}
                      </a>
                    ) : (
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-violet-400 outline-none"
              />
              <textarea
                rows={3}
                placeholder="Additional message (optional)"
                className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-violet-400 outline-none resize-none"
              />
            </div>
            <div className="flex justify-end gap-2 mt-5">
              ...
              <button className="px-5 py-2 text-xs font-bold bg-violet-600 text-white rounded-xl hover:bg-violet-700 flex items-center gap-1.5">
